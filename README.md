...
Different systems, identical failure point. Keep is an instrument for producing documented notice of a pattern, which is what these laws ask for and what nothing currently supplies.
...
**Everything is sealed before it moves, including on disk.** The record is encrypted with AES-GCM under a key derived by PBKDF2 (400,000 iterations, SHA-256) from the Keep code. Browser storage holds ciphertext only, so opening developer tools on a shared school computer reveals nothing. The key exists in page memory and nowhere else; a refresh requires the code again.
